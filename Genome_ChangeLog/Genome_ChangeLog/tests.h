...
void mutation_delete(int ind, int shift, std::map<int, std::pair<int, bool>>& r_map, std::unordered_map<int, std::vector<int>>& segments_log);
...
//        mutation_delete(3, 1, r_map, insertions);
...
//        mutation_delete(0, 1, r_map, insertions);
...
//        mutation_delete(genome.size() - 1, 1, r_map, insertions); // Implicit conversion loses integer precision: 'unsigned long' to 'int'?
    ...
    GIVEN("Original genome and empty change log") {
        ...
        WHEN("ins 4 sites at ind 2 && ins 2 sites at ind 3 && del 3 sites at ind 1 && del 4 sites at ind 3") {
            ...
            THEN("both offspring genome and change log are updated") {
                ...
                std::vector<int> expected_offspring{21, 6, 2, 12};
